...
from HuDA_loadData import Huda_process_lt, Huda_process_rt
...
def data_generator(batch_size):  # x1:source x2:target y1:source_label y2:target_label
    while 1:
        trainX_rt, trainy_rt, _, _ = Huda_process_rt.preprocess()
        trainX_lt, _, _, _ = Huda_process_lt.preprocess()
...
def val_generator(batch_size):  # x1:source x2:target y1:source_label y2:target_label
    _, _, testx_rt, testy_rt = Huda_process_rt.preprocess()
    _, _, testx_lt, testy_lt = Huda_process_lt.preprocess()
...
def run():
    ...
    '''
    '获取训练集和测试集数据'
    trainX_rt, trainy_rt, testx_rt, testy_rt = Huda_process_rt.preprocess()
    _, _, testx_lt, testy_lt = Huda_process_lt.preprocess()
    trainX_rt = trainX_rt[:19000]
    trainy_rt = trainy_rt[:19000]
    testx_rt = testx_rt[:8000]
    ...
    '构建迁移学习模型'
    base_model = keras.models.load_model('huda_best_model_SGD.h5')
    ...
    class evaluate (keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if self.val_accuracy>0.4:
                model.save('best_Huda_transfer_model.h5')
    ...
    #画出混淆矩阵
    confus_model=keras.models.load_model('best_Huda_transfer_model.h5')
